...
alter table public.whale_manager_holdings_snapshot
  alter column cost drop not null,
  alter column gap_pct drop not null;

alter table public.whale_manager_holdings_snapshot
  add column if not exists gap_known boolean not null default true,
  add column if not exists gap_reason text;

update public.whale_manager_holdings_snapshot
set
  gap_known = case when gap_pct is null then false else true end,
  gap_reason = case when gap_pct is null then coalesce(gap_reason, 'no_previous_cost_basis') else null end,
  cost = case when gap_pct is null then null else cost end
where true;
...
begin
  ...
  computed_holdings as (
    select
      ...
      case
        when coalesce(n.previous_shares, 0) > 0 and coalesce(n.previous_value_usd_thousands, 0) > 0 and n.shares > 0 then true
        else false
      end as gap_known,
      case
        when coalesce(n.previous_shares, 0) > 0 and coalesce(n.previous_value_usd_thousands, 0) > 0 and n.shares > 0 then null
        else 'no_previous_cost_basis'
      end as gap_reason,
...
comment on function public.refresh_whale_snapshot_tables() is
  'Recomputes whale manager snapshots and tracks unknown gap semantics when previous cost basis is unavailable.';
