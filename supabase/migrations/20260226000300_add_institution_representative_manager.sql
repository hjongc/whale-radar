...
alter table public.institutions
  add column if not exists representative_manager text;
...
begin
  ...
  with latest_filings as (
    select
      ...
      i.cik,
      i.institution_name,
      i.representative_manager as institution_representative_manager,
      f.id as filing_id,
      f.accession_number,
      f.filing_date,
      f.report_period,
      f.filing_manager_name,
      row_number() over (
        partition by i.id
  ...
  latest_positions as (
    select
      ...
      l.cik,
      l.institution_name,
      l.institution_representative_manager,
      l.filing_manager_name,
      l.report_period,
      l.filing_date,
      l.accession_number,
  ...
  normalized as (
    select
      ...
      coalesce(
        nullif(trim(lp.institution_representative_manager), ''),
        nullif(trim(lp.filing_manager_name), ''),
        lp.institution_name
      ) as representative_manager,
      lp.report_period,
      lp.filing_date,
      lp.accession_number,
      lp.ticker,
  ...
  manager_totals as (
    ...
    from normalized
    group by manager_id, manager_name, representative_manager, report_period, filing_date
  ),
  ranked_managers as (
    select
      manager_id,
...
comment on function public.refresh_whale_snapshot_tables() is
  'Recomputes whale manager snapshots and fills representative manager from institution override or filing manager metadata.';
